...
2. 安装本脚本 `fluxdown.user.js`，任选其一：
   - **从 FluxDown 应用内**：打开 FluxDown → 设置 → 本地服务 → 「复制油猴脚本」，
     在 Tampermonkey「添加新脚本」中粘贴保存；
   - **从文件**：用脚本管理器打开本仓库的 `userscript/fluxdown.user.js`。
3. 确保 FluxDown 桌面端正在运行，且「设置 → 本地服务」已开启（默认开启，端口 17800）。
...
| 项 | 说明 |
|---|---|
| 端口 | 须与 FluxDown「设置 → 本地服务 → 监听端口」一致（默认 17800） |
| Token | 仅当你在 FluxDown 设置了访问 Token 时才需要填写（菜单 → 设置 Token） |
...
FluxDown 本地服务额外暴露 `POST /jsonrpc` 的 **aria2 JSON-RPC 兼容端点**
